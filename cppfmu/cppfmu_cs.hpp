...
{
public:
    SlaveInstance():m_time{0.0},
		    m_state{modelInstantiated},
		    m_eventInfo{fmiFalse,fmiFalse,fmiFalse,fmiFalse,fmiFalse,0.0}
    {
    }
    ...
    virtual bool DoStep(
        ...
        fmiBoolean newStep,
        fmiReal& endOfStep) = 0;
    virtual void SetRealInputDerivatives(const fmiValueReference vr[],
					 std::size_t nvr,
					 const fmiInteger order[],
					 const fmiReal value[]);
    virtual void GetRealOutputDerivatives(const fmiValueReference vr[],
					  std::size_t nvr,
					  const fmiInteger order[],
					  fmiReal value[]) const;  
    // Members to monitor instance
    inline ModelState &modelState(void) {
        return m_state;
    }
    inline fmiBoolean &iterationConverged(void) {
        return m_eventInfo.iterationConverged;
    }
    inline fmiBoolean &stateValueReferencesChanged(void) {
        return m_eventInfo.stateValueReferencesChanged;
    }
    inline fmiBoolean &stateValuesChanged(void) {
        return m_eventInfo.stateValuesChanged;
    }
    inline fmiBoolean &terminateSimulation(void) {
        return m_eventInfo.terminateSimulation;
    }
    inline fmiBoolean &upcomingTimeEvent(void) {
        return m_eventInfo.upcomingTimeEvent;
    }
    inline fmiReal &nextEventTime(void) { 
        return m_eventInfo.nextEventTime;
    }
    void clearEventInfo(void){
        m_eventInfo.iterationConverged = fmiFalse;
        m_eventInfo.stateValueReferencesChanged = fmiFalse;
        m_eventInfo.stateValuesChanged = fmiFalse;
        m_eventInfo.terminateSimulation = fmiFalse;
        m_eventInfo.upcomingTimeEvent = fmiFalse;
